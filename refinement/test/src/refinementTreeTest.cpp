...
RefinementTreeTest::TEST_CTOR( SizeTest, "number of leaves after expansion" );
...
bool RefinementTreeTest::SizeTest::check() const
{
    ...
    if( graph::size( mpRtree->graph() ) != 1 + (EXPANSION_SIZE - 1) * mExpansionCounter )
    {
	D( std::cout << "bad number of nodes " << graph::size( mpRtree->graph() ) << " after " << mExpansionCounter << " expansions" << std::endl; );
	return false;
    }
    return true;
}
...
bool RefinementTreeTest::IntersectionTest::check() const
{
	...
	{
	    D( printNodeValue( nval ) );
	    D( std::cout << " is not returned as intersection of " << smallerBox << " even though it is contained inside" << std::endl; );
	    return false;
	}
	if( !intersects && iImg != imageSmallerBox.end() )
	{
	    D( printNodeValue( nval ) );
	    D( std::cout << " is returned as intersection with " << smallerBox << " even though it is not contained inside" << std::endl; );
	    return false;
	}
    }
    return true;
